...
public interface ToursService {
    ...
    // CONSULTAS HQL
    List<Purchase> getAllPurchasesOfUsername(String username); //Old -Ivy
    List<User> getUserSpendingMoreThan(float mount); //Old -Ivy
    List<User> getUsersWithNumberOfPurchases(int number); //New
    List<Supplier> getTopNSuppliersInPurchases(int n); //Old -Ivy
    List<Supplier> getTopNSuppliersItemsSold(int n); //New
    List<Purchase> getTop10MoreExpensivePurchasesWithServices(); //Old, tweaked title -Ivy
    List<Route> getTop3RoutesWithMoreStops(); //New
    //
    List<User> getTop5UsersMorePurchases(); //Old -Fabri
    Long getCountOfPurchasesBetweenDates(Date start, Date end); //Old -Fabri
    List<Route> getRoutesWithStop(Stop stop); //Old -Fabri
    List<Purchase> getPurchaseWithService(Service service); //New
    Long getMaxStopOfRoutes(); // Old -Fabri
    Long getMaxServicesOfSupplier(); //New
    List<Route> getRoutsNotSell(); //Old -Fabri
    //
    List<Route> getTop3RoutesWithMaxAverageRating(); //Old, tweaked title -Franco
    List<Route> getRoutesWithMinRating(); //Old -Franco
    Service getMostDemandedService(); //Old -Franco
    Route getMostBestSellingRoute(); //New
    List<Service> getServiceNoAddedToPurchases(); //Old -Franco
    List<TourGuideUser> getTourGuidesWithRating1(); //Old -Franco
    DriverUser getDriverUserWithMoreRoutes(); //New
}
